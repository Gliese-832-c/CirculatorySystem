    ...
    public static void addTooltipToValidConsumables(ItemTooltipEvent event) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            ...
            for (ConsumableEffect consumableEffect : consumable.consumableEffects) {
                event.getToolTip().add("§b" + I18n.format(StatusTrackers.getSystemTypeFromKey(consumableEffect.key).chatInfoMessage) + ": " + getValueColorizedPercentageConsumable(consumableEffect.amount) + (consumableEffect.isRelative ? "§7 - " + I18n.format("tooltip." + CirculatorySystem.MOD_ID + ".is_relative") : ""));
            }
        } else {
